...
const templateText = `
// Code generated by protoc-gen-msg-type. DO NOT EDIT.
package msgtype
...
func (r *MessageRegistry) registerMessage(msg interface{}, msgType uint32, name string) {  
    ...
    if existingType, exists := r.messageTypes[msgTyp]; exists {
        if existingType != msgType {
            panic(fmt.Sprintf("message type conflict for %s", name))
        }
        return
    }
...
func toStructName(name string) string {
	...
	for _, part := range parts {
		if part != "REQ" && part != "RSP" {
			result = append(result, cases.Title(language.Und).String(strings.ToLower(part)))
		}
	}
